...
### 3.4 Pod Resource Limit

The CPU and memory required by a pod can be set as below in the `3_4/flask-pod.yaml`:
...
### 3.4.1 Namespace Resource Limit
...
`flask-pod-2.yaml`, `flask-pod-3.yaml`, and `flask-pod-4.yaml` are configure files similar to `flask-pod.yaml`, and being only different in `metadata.name` and `containers.name`. Here we only display `flask-pod-2.yaml` as an example. Change the `metadata.name` and `containers.name` in the file can convert it to  `flask-pod-N.yaml`.
...
To test `Priority`, we first create two priority classes, `high` and `low`. They are applied to the pods by the parameter `priorityClassName` in `busybox-deploy-1.yaml` and `busybox-deploy-2.yaml`.
...
In a K8s cluster with one node with 16G memory. we apply `busybox-deploy-1.yaml`, five replicas will be created as normal. They request 5*3G memory in total.

```shell
$ kubectl apply -f busybox-deploy-1.yaml 
deployment.apps/busybox-deploy-1 created
$ kubectl get pods -n busybox-ns
NAME                                READY   STATUS    RESTARTS   AGE
busybox-deploy-1-774d88797c-4rqlk   1/1     Running   0          15s
busybox-deploy-1-774d88797c-857zh   1/1     Running   0          15s
busybox-deploy-1-774d88797c-tvvnf   1/1     Running   0          15s
busybox-deploy-1-774d88797c-v9cfp   1/1     Running   0          15s
busybox-deploy-1-774d88797c-zmv6c   1/1     Running   0          15s
```

Then we apply `busybox-deploy-2.yaml`. Because the 5G memory request of the pod is higher than the available memory in the node, the new `busybox-deploy-2` is currently pending. We can also see some pods from `busybox-deploy-1` are terminating, as they have lower priority, and must be evicted to make room for the new pods with higher priority.

```shell
$ kubectl apply -f busybox-deploy-2.yaml 
deployment.apps/busybox-deploy-2 created
$ kubectl get pods -n busybox-ns
NAME                                READY   STATUS        RESTARTS   AGE
busybox-deploy-1-774d88797c-4rqlk   1/1     Terminating   0          91s
busybox-deploy-1-774d88797c-857zh   1/1     Running       0          91s
busybox-deploy-1-774d88797c-kbffz   0/1     Pending       0          10s
busybox-deploy-1-774d88797c-s9vlf   0/1     Pending       0          10s
busybox-deploy-1-774d88797c-tvvnf   1/1     Running       0          91s
busybox-deploy-1-774d88797c-v9cfp   1/1     Terminating   0          91s
busybox-deploy-1-774d88797c-zmv6c   1/1     Running       0          91s
busybox-deploy-2-589d74f9c8-2nljt   0/1     Pending       0          10s
```

After a while, the eviction of `busybox-deploy-1` pods is finished, and the `busybox-deploy-2` pod turns into `Running` status.

```shell
$ kubectl get pods -n busybox-ns
NAME                                READY   STATUS    RESTARTS   AGE
busybox-deploy-1-774d88797c-857zh   1/1     Running   0          2m24s
busybox-deploy-1-774d88797c-kbffz   0/1     Pending   0          63s
busybox-deploy-1-774d88797c-s9vlf   0/1     Pending   0          63s
busybox-deploy-1-774d88797c-tvvnf   1/1     Running   0          2m24s
busybox-deploy-1-774d88797c-zmv6c   1/1     Running   0          2m24s
busybox-deploy-2-589d74f9c8-2nljt   1/1     Running   0          63s
```