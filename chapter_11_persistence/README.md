...
#### `reclaimPolicy` in StorageClass

Controls what happens to PersistentVolumes (PVs) when they are released (i.e., when the associated PersistentVolumeClaim is deleted). Here are three options:
1. Retain: When a PVC is deleted, the PV remains in the system and the storage asset is kept.
2. Delete: When a PVC is deleted, both the PV and the associated storage asset are automatically deleted. Ideal for temporary storage.

As we do not want to lose data in real production, we should use `Retain`. As a result, we need to manually delete the PV when we are done with it. It always happens when restarting a helm release.

#### `accessModes` in PersistentVolumeClaim

Determines how the volume can be mounted, have the following values:

1. ReadWriteOnce (RWO): The volume can be mounted as read-write by a single node. Work for writing to a block storage.
2. ReadOnlyMany (ROX): The volume can be mounted as read-only by many nodes. Useful for multiple pods reading the same configuration file/dataset.
3. ReadWriteMany (RWX): The volume can be mounted as read-write by many nodes. Typically used with network filesystems like CephFS, or cloud equivalents.
4. ReadWriteOncePod: The volume can be mounted as read-write by a single pod. Available in Kubernetes v1.22+.

#### `volumeHandle` in PersistentVolume

The unique identifier for a volume in the presistent system. For example, if we have multiple disk with different name mounted on a server, we can use `volumeHandle` to point our PV to a specific hardware disk.
...
```shell
$ docker pull swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/csi-provisioner:v3.5.0
$ docker tag  swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/csi-provisioner:v3.5.0  registry.k8s.io/sig-storage/csi-provisioner:v3.5.0
$ docker pull swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/csi-resizer:v1.8.0
$ docker tag  swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/csi-resizer:v1.8.0  registry.k8s.io/sig-storage/csi-resizer:v1.8.0
$ docker pull swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/csi-attacher:v4.3.0
$ docker tag  swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/csi-attacher:v4.3.0  registry.k8s.io/sig-storage/csi-attacher:v4.3.0
$ docker pull swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/csi-node-driver-registrar:v2.8.0
$ docker tag  swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/csi-node-driver-registrar:v2.8.0  registry.k8s.io/sig-storage/csi-node-driver-registrar:v2.8.0
$ docker pull swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/livenessprobe:v2.10.0
$ docker tag  swr.cn-north-4.myhuaweicloud.com/ddn-k8s/registry.k8s.io/sig-storage/livenessprobe:v2.10.0  registry.k8s.io/sig-storage/livenessprobe:v2.10.0
$ docker pull crpi-foj3lu39cfzgj05z.cn-shenzhen.personal.cr.aliyuncs.com/jerry-learn/seaweedfs-csi-driver:latest
...
```shell
$ kubectl apply -f seaweedfs-storage-static.yaml 
...
For the first time a pod mount to a pv, a pv directory will be created on the k8s worker at path `/var/lib/kubelet/plugins/kubernetes.io/csi/pv`.

If no pod uses this pv for a while, the directory will be removed, but the data are still kept in the storage device.
...
If we do not specify the `volumeName` of a PVC, but instead specify the `storageClassName`, a volume will be dynamically provisioned.

PS: Remember to `kubectl delete -f seaweedfs-storage-static.yaml` before running this demo.

```shell
$ kubectl apply -f seaweedfs-storage-dynamic.yaml
$ kubectl get pvc -n storage-demo-ns
NAME            STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS   AGE
...
Currently cannot find any log about the automatic generation of new PV.

The directory corresponding to the automatic generated PV is `/buckets/{pv_name}` by default. If `parameters.path` is set for the StorageClass, the directory will be exact `parameters.path`, no additional child directory will be created. 

If you want to have a child directory for each different PV, you need to customized the function `newMounter` in `pkg/driver/mounter.go` in the seaweedfs-csi-driver directory. It will be similar to the example below.
...
func newMounter(volumeID string, readOnly bool, driver *SeaweedFsDriver, volContext map[string]string) (Mounter, error) { 
  ...
  if !ok { 
    ...
    path = strings.ReplaceAll(path, "{volumeId}", volumeID) 
  } 
  glog.V(0).Infof("volume mount path: %s", path) c

  ollection, ok := volContext["collection"] 
  if !ok { 
    collection = volumeID 
  } 
